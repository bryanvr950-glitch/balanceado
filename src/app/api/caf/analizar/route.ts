...
const Schema = z.object({
  ...
  cafRegistroId:  z.string().uuid(),
})

const PROMPT = `Eres un asistente que extrae datos de planillas CAF (Control de Alimentación y Fertilización) de camaroneras.

Extrae EXACTAMENTE estos dos valores:
1. SALDO FINAL — el último valor de la fila "SALDOS". Este número está en KILOGRAMOS.
2. CONSUMO DEL ÚLTIMO DÍA — el último valor no-cero de la fila "CONSUMOS". En KILOGRAMOS.

Reglas:
- Devuelve los kg exactos como aparecen, NO dividas por 25.
- Si no puedes leer un valor claramente, devuelve null.
- Si hay un número tachado, ignóralo.

Responde SOLO con este JSON sin markdown:
{"saldo_kg": <número o null>, "consumo_kg": <número o null>, "notas": "<observación breve o vacío>"}`
...
export async function POST(req: NextRequest) {
  ...
  const { imageBase64, mediaType, campoId, productoId, cafRegistroId } = parsed.data
  ...
  try {
    ...
    await admin.from('caf_registros')
      .update({ estado: 'extraido', producto_id: productoId ?? null })
      .eq('id', cafRegistroId)
